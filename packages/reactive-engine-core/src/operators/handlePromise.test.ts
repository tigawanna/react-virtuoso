...
describe('handlePromise operator', () => {
  ...
  it('handles multiple promises in sequence', async () => {
    const source = Stream<Promise<number>>()
    const handled = e.pipe(
      source,
      e.handlePromise(
  ...
  it('handles promises that resolve to null/undefined', async () => {
    ...
    expect(history.slice(0, 2)).toEqual(['loading'])

    await waitForMicrotask()
    expect(history.slice(1)).toEqual(['resolved: null', 'resolved: undefined'])
  })
  ...
  it('handles promises that reject with non-Error objects', async () => {
    ...
    eng.pub(source, Promise.reject('custom error'))

    await waitForMicrotask()
    expect(history[1]).toBe(`error: custom error`)
  })
  ...
  it('handles promises that reject with null/undefined', async () => {
    ...
    await waitForMicrotask()
    expect(history.slice(1)).toEqual(['error: null', 'error: undefined'])
  })
