...
// Remove bogus "No Inherit Doc" headings generated by typedoc-plugin-no-inherit
const removeNoInheritDocHeadings = (content: string): string => {
  return content.replace(/^#{2,6} No Inherit Doc\n*/gm, '')
}
...
const mergeFilesByGroup = async (dir: string): Promise<void> => {
  ...
  for (const [group, groupFiles] of grouped) {
    ...
    // Process each file: clean up, increase heading levels, merge types into headings, and fix cross-links
    const processedFiles = groupFiles.map((f) => {
      const cleanedContent = removeNoInheritDocHeadings(f.content)
      const adjustedContent = increaseHeadingLevels(cleanedContent)
